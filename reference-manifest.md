...
1. **合成方式：乘积 vs 加权和。** 我方乘积意味着任一维趋零则整条记忆出局；斯坦福加权和允许"极高重要性"补偿"低相关性"（`gw=[0.5,3,2]` 里 importance 权重高达 3，就是让创伤性记忆穿透相关性过滤）。**但这个对比不能直接套到我方**：`REL` 为卡片显示层权重，不进决策函数；决策层记忆读取为 `f.mems` 直读（`engine2.js:84,137`）。斯坦福用 `gw` 争取的创伤穿透，我方已在直读路径上硬编码实现（骗补型"两期内被查→蛰伏"`:137`、激进型"兔死狐悲"`:84`），E8 探针即其证明。真实差距在**展示保真**：非骗补名角在 `expand`/`survive` 语境下，卡片"调取记忆"栏可能滤掉实际驱动决策的 `audit_hit` 记忆（rel=0.2/0.3），伤"决策链可追溯"。候选小卡：卡片 `recalled` 栏与决策函数实际咨询的记忆对齐（纯显示层，加法改动，过 GR）。
...
**若要动 `retrieve` / `REL`**：`retrieve` 零 rng 消耗、产物只流向 `f.lastCard.recalled`（`engine2.js:303→383`），改它**逐位不动任何轨迹**，不触发硬约束 4。
认知规则变体的正确归属是**假设注册表开关**（v3.1 §7③ / T-E3），**不是** `policy.x`——后者是政策编译通道，塞认知参数属层错位。
