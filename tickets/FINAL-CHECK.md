...
- ❌"30 秒换个产业整套跑起来"——换包主张范围**仅参数/剧本/名册**；引擎叙事文案硬编码光伏措辞，换不掉
  （R10 附带 2）。换包桥段**只用 `node scenarios/bus.js` 的 CLI 输出**演示；
  **禁止现场把 bus 的 CFG 灌进 arena 前端**——前端文案与单位标签仍是光伏的，当场穿帮。
...
**口径纪律（R9）**：若 pitch 只讲 0.460 不讲 0.628，**按未通过处理**——先自曝再翻盘才是这段的力量。

**必查项 · A11 补注在册**（R10 附带 1，2026-08-13 核准入册）

```bash
grep -n "demandBoostCap" specs/assumption-registry.md    # 期望：A11 行命中
```
若未命中：说明 specs 窗口尚未认领该补注。这是**一行文档编辑，本工单执行者可直接代办**，
待粘贴原文与理由见 `tickets/README.md`「致 specs 维护窗口」节。
**别让它悬着**——评委问"退坡怎么建模"时，这一行就是标准答案。

**换包桥段自检**（R10 附带 2，与第三关禁语清单第 5 条配套）
```bash
node scenarios/bus.js | tail -12    # 期望：四断言全 ✓ + 同种子双跑逐位一致 ✓
```
